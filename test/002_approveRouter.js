...
async function main() {
    ...
    const amtMIM = ethers.BigNumber.from("1000000000000000")
    const amtETH = ethers.BigNumber.from("1000000000000000")

    const SBR = await ethers.getContractFactory("SnowbearToken")
    const sbr = await SBR.attach(TIME_ADDRESS)
    await sbr.approve(ROUTER_ADDRESS, amtSBR)
    console.log("SBR -> approve -> ROUTER")

    const DAI = await ethers.getContractFactory("DAI")
    const dai = await DAI.attach(MIM_ADDRESS)
    await dai.approve(ROUTER_ADDRESS, amtMIM)
    console.log("DAI -> approve -> ROUTER")

    const WETH = await ethers.getContractFactory("WETH9")
    const weth = await WETH.attach(WETH_ADDRESS)
    await weth.approve(ROUTER_ADDRESS, amtETH)
    console.log("ETH -> approve ROUTER")
