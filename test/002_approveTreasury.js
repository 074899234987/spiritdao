...
async function main() {
    ...
    await dai.approve(TREASURY_ADDRESS, amtMIM)
    console.log("DAI -> approve -> TREASURY_ADDRESS")

    const WETH = await ethers.getContractFactory("WETH9")
    const weth = await WETH.attach(WETH_ADDRESS)
    await weth.approve(TREASURY_ADDRESS, amtETH)
    console.log("ETH -> approve TREASURY_ADDRESS")
